...
module.exports.run = async (client, message, args) => {
  ...
  if (!serverQueue || !serverQueue.songs) return message.channel.send(replyErr("`❌` I am not currently playing music."))
  serverQueue.songs = [];
  if (serverQueue.connection.dispatcher == null) return serverQueue.songs = [];
  serverQueue.connection.dispatcher.end();
  return message.channel.send(replyMsg("`⏹` Song stopped."))
